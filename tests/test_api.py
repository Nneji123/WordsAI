...
def test_sentiment_analysis():
    ...
    assert (
        response.json()
        == "The sentiment of the text is: Negative and the Score is: -0.57"
    )
...
def test_translation_error():
    ...
    assert (
        response.json()
        == "The translation of the text is: 'ESA' IS AN INVALID TARGET LANGUAGE . EXAMPLE: LANGPAIR=EN|IT USING 2 LETTER ISO OR RFC3066 LIKE ZH-CN. ALMOST ALL LANGUAGES SUPPORTED BUT SOME MAY HAVE NO CONTENT"
    )
